...
class Overlap_jj:
	...
	def __init__(self):
		...
		self.bottom_spacing_jj = None
		self.top_spacing_jj = None
	...
	def generateCell(self, **kwargs):
		...
		# add bottom layer
		bottom_electrode = gdstk.rectangle((-litho_overlap/2.0,-(height/2.0+electrode_height_difference)), (top_spacing/2.0+width,(height/2.0+electrode_height_difference)), layer=jj_bottom_layer)
		cell_out.add(bottom_electrode.copy().translate(x_position,y_position))
		x_position += top_spacing/2.0

		# add top layer
		top_electrode = gdstk.rectangle((0,-height/2.0), (bottom_spacing/2.0+width+litho_overlap/2.0,height/2.0), layer=jj_top_layer)
		cell_out.add(top_electrode.copy().translate(x_position,y_position))
		x_position += bottom_spacing/2.0+width
...
class Overlap_capa:
	...
	def generateCell(self, **kwargs):
		...
		# add bottom layer 
		bottom_electrode = gdstk.rectangle((-litho_overlap/2,-(height/2+electrode_height_difference)), (top_spacing/2+width,(height/2+electrode_height_difference)), layer=capa_bottom_layer)
		cell_out.add(bottom_electrode.copy().translate(x_position,y_position))
		x_position += top_spacing/2
		...
		if change_area_capa:
			...
			diff_height_capa = 0
			diff_width=0
		top_electrode = gdstk.rectangle((-diff_width,-(height+diff_height_capa)/2), (bottom_spacing/2+width+litho_overlap/2,(height+diff_height_capa)/2), layer=capa_top_layer)
		cell_out.add(top_electrode.copy().translate(x_position,y_position))
		x_position += bottom_spacing/2+width
...
class Resistor:
	...
	def __init__(self):
		...
		self.length_resistor = None
		self.width_resistor = None

		self.resistor_layer = 1
	...
	def generateCell(self, **kwargs):
		...
		length = self.length_resistor
		width = self.width_resistor

		resistor_layer = self.resistor_layer

		# generate cell to add features to
		cell_out = gdstk.Cell("SingleResistor")

		x_position = 0
		y_position = 0

		# add resistor 
		resistor = gdstk.rectangle((-litho_overlap/2,-width/2), (length,width/2), layer=resistor_layer)
		cell_out.add(resistor.copy().translate(x_position,y_position))

		x_position += length
		y_position += width/2

		self.x_size_of_cell = x_position
		self.y_size_of_cell = y_position
...
class Pad_TWPA_inv_microstrip:
	...
	def Generate_Pad_TWPA(self):
		...
		bonding_pad = gdstk.rectangle((x_dice_gap,-bonding_pad_width/2.0), (x_dice_gap + bonding_pad_length,bonding_pad_width/2.0), layer=pad_layer)
		...
		cell.add(connecting_wire.copy().translate(x_dice_gap + bonding_pad_length,0))

		x_len += connecting_line_length

		top_ground_pad_window = gdstk.rectangle((0,-bonding_pad_width/2.0-bonding_pad_gap_y), (x_dice_gap+bonding_pad_length+bonding_pad_gap_x,bonding_pad_width/2.0+bonding_pad_gap_y), layer=ground_layer)
...
class Pad_TWPA_CRLH_V02:
	...
	def __init__(self):
		...
		self.layer_2      = 2
		self.layer_ground = 3
		self.layer_capa_top_electrode = 7
		self.layer_shift  = 0
	...
	def Generate_Pad_TWPA(self):
		...
		capacitance_along_connecting_wire_width_ground = 0
		...
		connecting_wire_layer = layer_shift + self.layer_2
		ground_layer = layer_shift + self.layer_ground
		capa_top_layer = layer_shift + self.layer_capa_top_electrode
		...
		ground_pad_window = gdstk.rectangle((0,-bonding_pad_width/2.0-bonding_pad_gap_y), (x_dice_gap+bonding_pad_length+bonding_pad_gap_x,bonding_pad_width/2.0+bonding_pad_gap_y), layer=ground_layer)
		cell.add(ground_pad_window.copy().translate(0,0))
		top_ground_pad_window = gdstk.rectangle((0,-bonding_pad_width/2.0-bonding_pad_gap_y), (x_dice_gap+bonding_pad_length+bonding_pad_gap_x,bonding_pad_width/2.0+bonding_pad_gap_y), layer=capa_top_layer)
		cell.add(top_ground_pad_window.copy().translate(0,0))

		y_len_top += bonding_pad_gap_y
		y_len_bottom += bonding_pad_gap_y
		
		ground_connecting_wire_window = gdstk.rectangle((x_dice_gap,-connecting_line_width/2.0-connecting_wire_gap), (connecting_line_length - bonding_pad_gap_x + x_dice_gap,connecting_line_width/2.0+connecting_wire_gap), layer=ground_layer)
		top_ground_connecting_wire_window = gdstk.rectangle((x_dice_gap,-connecting_line_width/2.0-connecting_wire_gap), (connecting_line_length - bonding_pad_gap_x + x_dice_gap,connecting_line_width/2.0+connecting_wire_gap), layer=capa_top_layer)

		ground_wire_for_capa_along_connecting_wire = gdstk.rectangle((x_dice_gap,-connecting_line_width/2.0-connecting_wire_gap-1), (capacitance_along_connecting_wire_width_ground + x_dice_gap,connecting_line_width/2.0+connecting_wire_gap+1), layer=ground_layer)
		top_ground_wire_for_capa_along_connecting_wire = gdstk.rectangle((x_dice_gap,-connecting_line_width/2.0-connecting_wire_gap-1), (capacitance_along_connecting_wire_width + x_dice_gap,connecting_line_width/2.0+connecting_wire_gap+1), layer=capa_top_layer)
		
		ground_connecting_wire_window = gdstk.boolean(ground_connecting_wire_window,ground_wire_for_capa_along_connecting_wire.copy(),'not',layer=ground_layer)
		top_ground_connecting_wire_window = gdstk.boolean(top_ground_connecting_wire_window,top_ground_wire_for_capa_along_connecting_wire.copy(),'not',layer=capa_top_layer)

		for i in range(int(capacitance_along_connecting_wire_pitch_first),int(connecting_line_length),int(capacitance_along_connecting_wire_pitch)):
			
			ground_connecting_wire_window = gdstk.boolean(ground_connecting_wire_window,ground_wire_for_capa_along_connecting_wire.copy().translate(i,0),'not',layer=ground_layer)
			top_ground_connecting_wire_window = gdstk.boolean(top_ground_connecting_wire_window,top_ground_wire_for_capa_along_connecting_wire.copy().translate(i,0),'not',layer=capa_top_layer)
			
		for i in range(0,len(ground_connecting_wire_window)):
			
			cell.add(ground_connecting_wire_window[i].copy().translate(bonding_pad_length+bonding_pad_gap_x,0))
...
class Pad_TWPA_CRLH_V03:
	...
	def __init__(self):
		...
		self.layer_1      = 1
		self.layer_2      = 2
		self.layer_jj_bottom_electrode = 1
		self.layer_jj_top_electrode = 2
		self.layer_capa_top_electrode = 3
		self.layer_shift  = 0
	...
	def Generate_Pad_TWPA(self):
		...
		pad_layer = layer_shift + self.layer_1
		connecting_wire_layer = layer_shift + self.layer_2
		jj_bottom_layer = layer_shift + self.layer_jj_bottom_electrode
		jj_top_layer = layer_shift + self.layer_jj_top_electrode
		capa_top_layer = layer_shift + self.layer_capa_top_electrode
		...
		## Add pad and pad window

		ground_pad_window = gdstk.rectangle((0,-bonding_pad_width/2.0-bonding_pad_gap_y), (x_dice_gap+bonding_pad_length+bonding_pad_gap_x,bonding_pad_width/2.0+bonding_pad_gap_y), layer=jj_bottom_layer)

		bonding_pad = gdstk.rectangle((x_dice_gap,-bonding_pad_width/2.0), (x_dice_gap + bonding_pad_length,bonding_pad_width/2.0), layer=jj_bottom_layer)
		bonding_pad_neg = gdstk.boolean(ground_pad_window, bonding_pad, 'not', layer=jj_bottom_layer)
		...
		connecting_wire = gdstk.rectangle((-pad_wire_overlap,-connecting_line_width/2.0), (connecting_line_length,connecting_line_width/2.0), layer=jj_bottom_layer)
		bonding_pad_neg = gdstk.boolean(bonding_pad_neg, connecting_wire.copy().translate(x_dice_gap + bonding_pad_length,0), 'not', layer=jj_bottom_layer)

		for i in range(0,len(bonding_pad_neg)):
			
			cell.add(bonding_pad_neg[i].copy().translate(0,0))
		...
		## Add connecting wire window
		
		ground_connecting_wire_window = gdstk.rectangle((x_dice_gap,-connecting_line_width/2.0-connecting_wire_gap), (connecting_line_length - bonding_pad_gap_x + x_dice_gap,connecting_line_width/2.0+connecting_wire_gap), layer=jj_bottom_layer)
		ground_connecting_wire_window_neg = gdstk.boolean(ground_connecting_wire_window.copy().translate(bonding_pad_length+bonding_pad_gap_x,0), connecting_wire.copy().translate(x_dice_gap + bonding_pad_length,0), 'not', layer=jj_bottom_layer)

		for i in range(0,len(ground_connecting_wire_window_neg)):
				
			cell.add(ground_connecting_wire_window_neg[i].copy().translate(0,0))
...
class Pad_TWPA_CRLH_V04:
	...
	def Generate_Pad_TWPA(self):
		...
		ground_pad_window = gdstk.rectangle((0,-bonding_pad_width/2.0-bonding_pad_gap_y), (x_dice_gap+bonding_pad_length+bonding_pad_gap_x,bonding_pad_width/2.0+bonding_pad_gap_y), layer=jj_bottom_layer)
		top_layer_capa_pad_window = gdstk.rectangle((0,-bonding_pad_width/2.0-bonding_pad_gap_y), (x_dice_gap+bonding_pad_length+bonding_pad_gap_x,bonding_pad_width/2.0+bonding_pad_gap_y), layer=capa_top_layer)
		cell.add(top_layer_capa_pad_window.copy().translate(0,0))
...
class CRLH_V01_cell:
	'''
	Class to generate the unit cell for a Composite Right-Left-Handed Josephson Transmission Line
	  There is one function:
		2) generateCell: generate the unit cell of CRLH_V01_JTL with one capacitor connected in serie to a chain of junction, 
		itself connected to a capacitor and a chain of junction to ground, using the classes Overlap_jj and Overlap_capa
		The variable self.x_size_of_cell and self.y_size_of_cell store sizes of the structure for global use
		Note that it stores the variables self.x_size_of_cell and self.y_size_of_cell of the classes Overlap_jj and Overlap_capa in the
		variables:
		1) x_size_of_capa
	...
	def __init__(self):
		...
		self.width_jj_line = None
		self.height_jj_line = None
		self.spacing_jj_line = None
		self.bottom_spacing_jj_line = None
		self.top_spacing_jj_line = None
		self.electrode_height_difference_JJ_line = None
		self.number_of_junctions_line = None

		self.width_capa_line = None
		self.height_capa_line = None
		self.spacing_capa_line = None
		self.bottom_spacing_capa_line = None
		self.top_spacing_capa_line = None
		self.electrode_height_difference_capa_line = None
		self.number_of_capacitors_line = None

		self.width_jj_ground = None
		self.height_jj_ground = None
		self.spacing_jj_ground = None
		self.bottom_spacing_jj_ground = None
		self.top_spacing_jj_ground = None
		self.electrode_height_difference_JJ_ground = None
		self.number_of_junctions_ground = None

		self.width_capa_ground = None
		self.height_capa_ground = None
		self.spacing_capa_ground = None
		self.bottom_spacing_capa_ground = None
		self.top_spacing_capa_ground = None
		self.electrode_height_difference_capa_ground = None
		self.number_of_capacitors_ground = None
	...
	def generateCell(self, **kwargs):
		...
		max_width_capa_line = 6.7
		max_height_capa_line = 20.0
		max_width_capa_ground = 6.7
		max_height_capa_ground = 20.0

		PPC_line_inst = Overlap_capa()
		PPC_line_inst.litho_overlap = self.litho_overlap
		PPC_line_inst.etching_offset = self.etching_offset
		PPC_line_inst.y_low_current_ground = self.y_low_current_ground
		PPC_line_inst.width_capa = self.width_capa_line
		PPC_line_inst.height_capa = self.height_capa_line
		PPC_line_inst.spacing_capa = self.spacing_capa_line
		PPC_line_inst.bottom_spacing_capa = self.bottom_spacing_capa_line
		# PPC_line_inst.top_spacing_capa = self.top_spacing_capa_line
		PPC_line_inst.top_spacing_capa = 2*(max_width_capa_line-self.width_capa_line+self.electrode_height_difference_capa_line)
		PPC_line_inst.electrode_height_difference = self.electrode_height_difference_capa_line
		PPC_line_inst.capa_bottom_layer = self.capa_bottom_layer
		PPC_line_inst.capa_top_layer = self.capa_top_layer
		self.PPC_line = PPC_line_inst.generateCell()

		JJ_line_inst = Overlap_jj()
		JJ_line_inst.litho_overlap = self.litho_overlap
		JJ_line_inst.etching_offset = self.etching_offset
		JJ_line_inst.y_low_current_ground = self.y_low_current_ground
		JJ_line_inst.width_jj = self.width_jj_line
		JJ_line_inst.height_jj = self.height_jj_line
		JJ_line_inst.spacing_jj = self.spacing_jj_line
		JJ_line_inst.bottom_spacing_jj = self.bottom_spacing_jj_line
		JJ_line_inst.top_spacing_jj = self.top_spacing_jj_line
		JJ_line_inst.electrode_height_difference = self.electrode_height_difference_JJ_line
		JJ_line_inst.jj_bottom_layer = self.jj_bottom_layer
		JJ_line_inst.jj_top_layer = self.jj_top_layer
		self.JJ_line = JJ_line_inst.generateCell()

		PPC_ground_inst = Overlap_capa()
		PPC_ground_inst.litho_overlap = self.litho_overlap
		PPC_ground_inst.etching_offset = self.etching_offset
		PPC_ground_inst.y_low_current_ground = self.y_low_current_ground
		PPC_ground_inst.width_capa = self.width_capa_ground + self.etching_offset
		PPC_ground_inst.height_capa = self.height_capa_ground
		PPC_ground_inst.spacing_capa = self.spacing_capa_ground
		PPC_ground_inst.bottom_spacing_capa = self.bottom_spacing_capa_ground
		# PPC_ground_inst.top_spacing_capa = self.top_spacing_capa_ground
		PPC_ground_inst.top_spacing_capa = 2*(max_width_capa_ground-self.width_capa_ground+self.electrode_height_difference_capa_ground)
		PPC_ground_inst.electrode_height_difference = self.electrode_height_difference_capa_ground
		PPC_ground_inst.capa_bottom_layer = 101
		PPC_ground_inst.capa_top_layer = self.capa_top_layer
		PPC_to_ground = PPC_ground_inst.generateCell()
		PPC_ground_inst.width_capa = self.width_capa_ground
		PPC_ground_inst.capa_bottom_layer = self.capa_bottom_layer
		self.PPC_ground = PPC_ground_inst.generateCell()

		JJ_ground_inst = Overlap_jj()
		JJ_ground_inst.litho_overlap = self.litho_overlap
		JJ_ground_inst.etching_offset = self.etching_offset
		JJ_ground_inst.y_low_current_ground = self.y_low_current_ground
		JJ_ground_inst.width_jj = self.width_jj_ground + self.etching_offset
		JJ_ground_inst.height_jj = self.height_jj_ground
		JJ_ground_inst.spacing_jj = self.spacing_jj_ground
		JJ_ground_inst.bottom_spacing_jj = self.bottom_spacing_jj_ground
		JJ_ground_inst.top_spacing_jj = self.top_spacing_jj_ground
		JJ_ground_inst.electrode_height_difference = self.electrode_height_difference_JJ_ground
		JJ_ground_inst.jj_bottom_layer = 101
		JJ_ground_inst.jj_top_layer = self.jj_top_layer
		JJ_to_ground = JJ_ground_inst.generateCell()
		JJ_ground_inst.width_jj = self.width_jj_ground
		JJ_ground_inst.jj_bottom_layer = self.jj_bottom_layer
		self.JJ_ground = JJ_ground_inst.generateCell()

		litho_overlap = self.litho_overlap
		etching_offset = self.etching_offset

		number_of_junctions_line = self.number_of_junctions_line
		number_of_capacitors_line = self.number_of_capacitors_line
		number_of_junctions_ground = self.number_of_junctions_ground
		number_of_capacitors_ground = self.number_of_capacitors_ground

		# generate cell to add features to
		cell_out = gdstk.Cell("CRLH_V01_cell")

		# add PPC in line
		x_pos = 0
		y_pos = 0
		x_increment = PPC_line_inst.x_size_of_cell

		for i in range(number_of_capacitors_line):
			if i%2 == 0:
				PPC_line_ref = gdstk.Reference(self.PPC_line, origin=(x_pos+i*x_increment,y_pos))
			else:
				PPC_line_ref = gdstk.Reference(self.PPC_line, origin=(x_pos+(i+1)*x_increment,y_pos), rotation=np.pi)
			cell_out.add(PPC_line_ref)

		# add chain of junctions in line
		x_pos += number_of_capacitors_line*PPC_line_inst.x_size_of_cell+litho_overlap/2
		y_pos = 0
		x_increment = JJ_line_inst.x_size_of_cell

		for i in range(number_of_junctions_line):
			if i%2 == 0:
				JJ_line_ref = gdstk.Reference(self.JJ_line, origin=(x_pos+i*x_increment,y_pos), rotation=0)
			else:
				JJ_line_ref = gdstk.Reference(self.JJ_line, origin=(x_pos+(i+1)*x_increment,y_pos), rotation=np.pi)
			cell_out.add(JJ_line_ref)

		# add center pad PPC
		x_pos += number_of_junctions_line*JJ_line_inst.x_size_of_cell
		y_pos += 0

		size_jj_to_ground = (self.number_of_junctions_ground-1) * (self.top_spacing_jj_ground/2 + self.width_jj_ground + self.bottom_spacing_jj_ground/2) + self.bottom_spacing_jj_ground/2
		size_capa_to_ground = (self.number_of_capacitors_ground-1) * (PPC_ground_inst.top_spacing_capa/2 + self.width_capa_ground + self.bottom_spacing_capa_ground/2) + self.bottom_spacing_capa_ground/2
		
		if size_jj_to_ground>size_capa_to_ground:
			add_spacing_jj = 0
			add_spacing_capa = size_jj_to_ground-size_capa_to_ground
		else:
			add_spacing_jj = size_capa_to_ground-size_jj_to_ground
			add_spacing_capa = 0

		# center_pad_ref = gdstk.rectangle((0,-(self.height_capa_line/2+self.electrode_height_difference_capa_line+add_spacing_capa)), ((self.height_capa_ground+2*self.electrode_height_difference_capa_ground),self.height_capa_line/2+self.electrode_height_difference_capa_line+add_spacing_jj), layer=self.capa_bottom_layer)
		# cell_out.add(center_pad_ref.copy().translate(x_pos,y_pos))
		
		# center_pad_1_ref = gdstk.rectangle((0,-(self.height_jj_line/2+self.electrode_height_difference_JJ_line)), ((self.height_capa_ground+2*self.electrode_height_difference_capa_ground),self.height_jj_line/2+self.electrode_height_difference_capa_line), layer=self.jj_bottom_layer)
		# center_pad_2_ref = gdstk.rectangle((-(self.height_jj_ground/2+self.electrode_height_difference_JJ_ground),-(self.height_capa_line/2+self.electrode_height_difference_capa_line+add_spacing_capa)), (self.height_jj_ground/2+self.electrode_height_difference_JJ_ground,self.height_capa_line/2+self.electrode_height_difference_capa_line+add_spacing_jj), layer=self.jj_bottom_layer)

		center_pad_1_ref = gdstk.rectangle((0,-2), ((self.height_capa_ground+2*self.electrode_height_difference_capa_ground),2), layer=self.jj_bottom_layer)
		center_pad_2_ref = gdstk.rectangle((-2,-(self.height_capa_line/2+self.electrode_height_difference_capa_line+add_spacing_capa)), (2,self.height_capa_line/2+self.electrode_height_difference_capa_line+add_spacing_jj), layer=self.jj_bottom_layer)

		cell_out.add(center_pad_1_ref.copy().translate(x_pos,y_pos))
		cell_out.add(center_pad_2_ref.copy().translate(x_pos+self.height_capa_ground/2+self.electrode_height_difference_capa_ground,y_pos))

		# add PPC to ground
		x_pos += self.height_capa_ground/2+self.electrode_height_difference_capa_ground
		y_pos -= self.height_capa_line/2+self.electrode_height_difference_capa_line+add_spacing_capa
		y_increment = PPC_ground_inst.x_size_of_cell

		for i in range(number_of_capacitors_ground):
			if i%2 == 0:
				PPC_ground_ref = gdstk.Reference(self.PPC_ground, origin=(x_pos,y_pos-i*y_increment), rotation=-np.pi/2)
			elif i == number_of_capacitors_ground-1:
				PPC_ground_ref = gdstk.Reference(PPC_to_ground, origin=(x_pos,y_pos-(i+1)*y_increment-etching_offset), rotation=np.pi/2)
			else:
				PPC_ground_ref = gdstk.Reference(self.PPC_ground, origin=(x_pos,y_pos-(i+1)*y_increment), rotation=np.pi/2)
			cell_out.add(PPC_ground_ref)
		
		# add chain of junctions to ground
		x_pos += 0
		y_pos += self.height_capa_line+2*self.electrode_height_difference_capa_line+add_spacing_capa+add_spacing_jj
		y_increment = JJ_ground_inst.x_size_of_cell

		for i in range(number_of_junctions_ground):
			if i%2 == 0:
				JJ_ground_ref = gdstk.Reference(self.JJ_ground, origin=(x_pos,y_pos+i*y_increment), rotation=np.pi/2)
			elif i == number_of_junctions_ground-1:
				JJ_ground_ref = gdstk.Reference(JJ_to_ground, origin=(x_pos,y_pos+(i+1)*y_increment+etching_offset), x_reflection=True, rotation=-np.pi/2)
			else:
				JJ_ground_ref = gdstk.Reference(self.JJ_ground, origin=(x_pos,y_pos+(i+1)*y_increment), x_reflection=True, rotation=-np.pi/2)
			cell_out.add(JJ_ground_ref)

		cell_out.flatten()

		self.x_size_of_capa = PPC_line_inst.x_size_of_cell
		self.y_size_of_capa = PPC_line_inst.y_size_of_cell
		self.x_size_of_jj = JJ_line_inst.x_size_of_cell
		self.y_size_of_jj = JJ_line_inst.y_size_of_cell
		self.x_size_of_cell = number_of_capacitors_line*PPC_line_inst.x_size_of_cell + number_of_junctions_line*JJ_line_inst.x_size_of_cell + self.height_capa_ground + 2*self.electrode_height_difference_capa_ground
		self.x_size_of_cell_tot = self.x_size_of_cell + 5
		self.y_size_of_cell = PPC_line_inst.height_capa/2 + PPC_line_inst.electrode_height_difference + max(size_jj_to_ground,size_capa_to_ground)
...
class CRLH_V02_cell:
	...
	Class to generate the unit cell for a Composite Right-Left-Handed Josephson Transmission Line
	  There is one function:
		2) generateCell: generate the unit cell of CRLH_V02_JTL with one capacitor connected in serie to a chain of junction, 
		itself connected to a capacitor and a chain of junction to ground, using the classes Overlap_jj and Overlap_capa
		The variable self.x_size_of_cell and self.y_size_of_cell store sizes of the structure for global use
		Note that it stores the variables self.x_size_of_cell and self.y_size_of_cell of the classes Overlap_jj and Overlap_capa in the
		variables:
	...
	def generateCell(self, **kwargs):
		...
		max_width_capa_line = 7.52
		max_height_capa_line = 20.0
		max_width_capa_ground = 3.76
		max_height_capa_ground = 20.0
		...
		length_feedline = JJ_ground_inst.height_jj+2*JJ_ground_inst.electrode_height_difference + 4.0
		width_feedline = 4.0
		...
		# generate cell to add features to
		cell_out = gdstk.Cell("CRLH_V02_cell")

		size_jj_to_ground = (self.number_of_junctions_ground-1) * (self.top_spacing_jj_ground/2 + self.width_jj_ground + self.bottom_spacing_jj_ground/2) + self.bottom_spacing_jj_ground/2
		self.x_size_of_cell = number_of_capacitors_line*PPC_line_inst.x_size_of_cell + number_of_junctions_line*JJ_line_inst.x_size_of_cell + max_width_capa_ground + self.top_spacing_capa_ground + length_feedline
		self.y_size_of_cell = (width_feedline/2 + size_jj_to_ground) * 2

		full_cell = gdstk.rectangle((-litho_overlap/2,-self.y_size_of_cell/2),(self.x_size_of_cell+litho_overlap/2,self.y_size_of_cell/2), layer=self.capa_top_layer)

		# add PPC in line
		bottom_electrode_capa_line = gdstk.rectangle((-litho_overlap/2,-(PPC_line_inst.height_capa/2+PPC_line_inst.electrode_height_difference)), (PPC_line_inst.top_spacing_capa/2+PPC_line_inst.width_capa,(PPC_line_inst.height_capa/2+PPC_line_inst.electrode_height_difference)), layer=self.capa_bottom_layer)
		top_electrode_capa_line = gdstk.rectangle((0,-PPC_line_inst.height_capa/2), (PPC_line_inst.bottom_spacing_capa/2+PPC_line_inst.width_capa+litho_overlap/2,PPC_line_inst.height_capa/2), layer=self.capa_top_layer)
		...
		for i in range(number_of_capacitors_line):
			if i%2 == 0:
				x_pos = 0
				y_pos = 0
				cell_out.add(bottom_electrode_capa_line.copy().rotate(0,[0,0]).translate(x_pos+i*x_increment,y_pos))

				x_pos += PPC_line_inst.top_spacing_capa/2
				y_pos += 0
				top_electrode_capa_line_pos = top_electrode_capa_line.copy().rotate(0,[0,0]).translate(x_pos+i*x_increment,y_pos)
				top_electrode_neg = gdstk.boolean(full_cell, top_electrode_capa_line_pos, "not", layer=self.capa_top_layer)

			else:
				x_pos = 0
				y_pos = 0
				cell_out.add(bottom_electrode_capa_line.copy().rotate(np.pi,[0,0]).translate(x_pos+(i+1)*x_increment,y_pos))

				x_pos -= PPC_line_inst.top_spacing_capa/2
				y_pos += 0
				top_electrode_capa_line_pos = top_electrode_capa_line.copy().rotate(np.pi,[0,0]).translate(x_pos+(i+1)*x_increment,y_pos)
				top_electrode_neg = gdstk.boolean(top_electrode_neg, top_electrode_capa_line_pos, "not", layer=self.capa_top_layer)

		x_pos = 0
		y_pos = 0

		# add chain of junctions in line
		x_pos += number_of_capacitors_line*PPC_line_inst.x_size_of_cell
		y_pos = 0
		x_increment = JJ_line_inst.x_size_of_cell
		...
		# add bottom layer PPC to ground
		x_pos += number_of_junctions_line*JJ_line_inst.x_size_of_cell
		y_pos += 0

		bottom_layer_ppc_ground = gdstk.rectangle((-litho_overlap/2,-PPC_ground_inst.height_capa/2), (max_width_capa_ground+self.top_spacing_capa_ground+litho_overlap/2,PPC_ground_inst.height_capa/2), layer=self.capa_bottom_layer)
		
		cell_out.add(bottom_layer_ppc_ground.copy().translate(x_pos,y_pos))

		# add top layer PPC to ground
		x_pos += (max_width_capa_ground + self.top_spacing_capa_ground)/2 - PPC_ground_inst.width_capa/2
		y_pos += 0
		
		top_electrode_capa_ground = gdstk.rectangle((0,-(width_feedline/2 + size_jj_to_ground + litho_overlap)), (PPC_ground_inst.width_capa,width_feedline/2+size_jj_to_ground+litho_overlap), layer=self.capa_top_layer)
		top_electrode_capa_ground_pos = top_electrode_capa_ground.translate(x_pos,y_pos)

		top_electrode_neg = gdstk.boolean(top_electrode_neg, top_electrode_capa_ground_pos, "not", layer=self.capa_top_layer)

		# add feedline
		x_pos += PPC_ground_inst.width_capa/2 + (max_width_capa_ground + self.top_spacing_capa_ground)/2
		y_pos += 0

		feedline = gdstk.rectangle((-litho_overlap/2,-(width_feedline/2+litho_overlap/2)), (length_feedline+litho_overlap/2,width_feedline/2+litho_overlap/2), layer=self.jj_bottom_layer)
		
		cell_out.add(feedline.copy().translate(x_pos,y_pos))
		
		# add chain of junctions to ground
		x_pos += length_feedline/2
		y_pos += width_feedline/2
		y_increment = JJ_ground_inst.x_size_of_cell
		...
		top_electrode_remove = gdstk.rectangle((0,0),(JJ_ground_inst.height_jj+2*JJ_ground_inst.electrode_height_difference, JJ_ground_inst.width_jj+JJ_ground_inst.electrode_height_difference+litho_overlap/2), layer=self.capa_top_layer)
		top_electrode_remove_pos = top_electrode_remove.copy().translate(x_pos-(JJ_ground_inst.height_jj/2+JJ_ground_inst.electrode_height_difference),y_pos+size_jj_to_ground)
		top_electrode_neg = gdstk.boolean(top_electrode_neg, top_electrode_remove_pos, 'or', layer=self.capa_top_layer)
		
		for poly in top_electrode_neg:
			cell_out.add(poly.copy())
		...
		self.x_size_of_jj = JJ_line_inst.x_size_of_cell
		self.y_size_of_jj = JJ_line_inst.y_size_of_cell
		self.x_size_of_cell = x_pos + length_feedline/2
		self.x_size_of_cell_tot = self.x_size_of_cell + 5
		self.y_size_of_cell = width_feedline/2 + size_jj_to_ground
...
class CRLH_V03_cell:
	...
	Class to generate the unit cell for a Composite Right-Left-Handed Josephson Transmission Line
	  There is one function:
		2) generateCell: generate the unit cell of CRLH_V04_JTL with one capacitor connected in serie to a chain of junction, 
		itself connected to a capacitor and a chain of junction to ground, using the classes Overlap_jj and Overlap_capa
		The variable self.x_size_of_cell and self.y_size_of_cell store sizes of the structure for global use
		Note that it stores the variables self.x_size_of_cell and self.y_size_of_cell of the classes Overlap_jj and Overlap_capa in the
		variables:
	...
	def generateCell(self, **kwargs):
		...
		max_width_capa_line = 6.70
		max_height_capa_line = 20.0
		max_width_capa_ground = 6.70
		max_height_capa_ground = 20.0
		...
		PPC_line_inst.top_spacing_capa = 2*(max_width_capa_line-self.width_capa_line+self.electrode_height_difference_capa_line)
		PPC_line_inst.electrode_height_difference = self.electrode_height_difference_capa_line
		PPC_line_inst.capa_bottom_layer = self.bottom_layer
		PPC_line_inst.capa_top_layer = self.capa_top_layer
		self.PPC_line = PPC_line_inst.generateCell()
		...
		JJ_line_inst.top_spacing_jj = self.top_spacing_jj_line
		JJ_line_inst.electrode_height_difference = self.electrode_height_difference_JJ_line
		JJ_line_inst.jj_bottom_layer = self.bottom_layer
		JJ_line_inst.jj_top_layer = self.jj_top_layer
		self.JJ_line = JJ_line_inst.generateCell()
		...
		PPC_to_ground = PPC_ground_inst.generateCell()
		PPC_ground_inst.width_capa = self.width_capa_ground
		PPC_ground_inst.capa_bottom_layer = self.bottom_layer
		self.PPC_ground = PPC_ground_inst.generateCell()
		...
		JJ_to_ground = JJ_ground_inst.generateCell()
		JJ_ground_inst.width_jj = self.width_jj_ground
		JJ_ground_inst.jj_bottom_layer = self.bottom_layer
		self.JJ_ground = JJ_ground_inst.generateCell()
		...
		# generate cell to add features to
		cell_out = gdstk.Cell("CRLH_V04_cell")

		size_jj_to_ground = (number_of_junctions_ground-1) * (JJ_ground_inst.top_spacing_jj/2 + JJ_ground_inst.width_jj + JJ_ground_inst.bottom_spacing_jj/2) + JJ_ground_inst.bottom_spacing_jj/2
		size_capa_to_ground = (number_of_capacitors_ground-1) * (PPC_ground_inst.top_spacing_capa/2 + PPC_ground_inst.width_capa + PPC_ground_inst.bottom_spacing_capa/2) + PPC_ground_inst.bottom_spacing_capa/2
		
		self.x_size_of_cell = number_of_capacitors_line*PPC_line_inst.x_size_of_cell + number_of_junctions_line*JJ_line_inst.x_size_of_cell + PPC_ground_inst.height_capa + 2*PPC_ground_inst.electrode_height_difference
		self.y_size_of_cell = (PPC_line_inst.height_capa/2 + PPC_line_inst.electrode_height_difference + size_jj_to_ground) * 2
		self.x_size_of_capa = PPC_line_inst.x_size_of_cell
		self.y_size_of_capa = PPC_line_inst.y_size_of_cell
		self.x_size_of_jj = JJ_line_inst.x_size_of_cell
		self.y_size_of_jj = JJ_line_inst.y_size_of_cell
		...
		bottom_electrode_neg = gdstk.rectangle((-litho_overlap/2,-self.y_size_of_cell/2),(self.x_size_of_cell+litho_overlap/2,self.y_size_of_cell/2), layer=self.bottom_layer)

		# add PPC in line
		bottom_electrode_capa_line = gdstk.rectangle((-litho_overlap/2,-(PPC_line_inst.height_capa/2+PPC_line_inst.electrode_height_difference)), (PPC_line_inst.top_spacing_capa/2+PPC_line_inst.width_capa,(PPC_line_inst.height_capa/2+PPC_line_inst.electrode_height_difference)), layer=self.bottom_layer)
		top_electrode_capa_line = gdstk.rectangle((0,-PPC_line_inst.height_capa/2), (PPC_line_inst.bottom_spacing_capa/2+PPC_line_inst.width_capa+litho_overlap/2,PPC_line_inst.height_capa/2), layer=self.capa_top_layer)
		...
		for i in range(number_of_capacitors_line):
			if i%2 == 0:
				bottom_electrode_capa_line_pos = bottom_electrode_capa_line.copy().rotate(0,[0,0]).translate(x_pos+i*x_increment,y_pos)
				bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, bottom_electrode_capa_line_pos, "not", layer=self.bottom_layer)

				cell_out.add(top_electrode_capa_line.copy().rotate(0,[0,0]).translate(x_pos+PPC_line_inst.top_spacing_capa/2+i*x_increment,y_pos))

			else:
				bottom_electrode_capa_line_pos = bottom_electrode_capa_line.copy().rotate(np.pi,[0,0]).translate(x_pos+(i+1)*x_increment,y_pos)
				bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, bottom_electrode_capa_line_pos, "not", layer=self.bottom_layer)

				cell_out.add(top_electrode_capa_line.copy().rotate(np.pi,[0,0]).translate(x_pos-PPC_line_inst.top_spacing_capa/2+(i+1)*x_increment,y_pos))

		# add chain of junctions in line
		x_pos += number_of_capacitors_line*PPC_line_inst.x_size_of_cell
		y_pos = 0

		bottom_electrode_JJ_line = gdstk.rectangle((-litho_overlap/2,-(JJ_line_inst.height_jj/2+JJ_line_inst.electrode_height_difference)), (JJ_line_inst.top_spacing_jj/2+JJ_line_inst.width_jj,(JJ_line_inst.height_jj/2+JJ_line_inst.electrode_height_difference)), layer=self.bottom_layer)
		top_electrode_JJ_line = gdstk.rectangle((0,-JJ_line_inst.height_jj/2), (JJ_line_inst.bottom_spacing_jj/2+JJ_line_inst.width_jj+litho_overlap/2,JJ_line_inst.height_jj/2), layer=self.jj_top_layer)

		x_increment = JJ_line_inst.x_size_of_cell
		for i in range(number_of_junctions_line):
			if i%2 == 0:
				bottom_electrode_JJ_line_pos = bottom_electrode_JJ_line.copy().rotate(0,[0,0]).translate(x_pos+i*x_increment,y_pos)
				bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, bottom_electrode_JJ_line_pos, "not", layer=self.bottom_layer)

				cell_out.add(top_electrode_JJ_line.copy().rotate(0,[0,0]).translate(x_pos+JJ_line_inst.top_spacing_jj/2+i*x_increment,y_pos))

			else:
				bottom_electrode_JJ_line_pos = bottom_electrode_JJ_line.copy().rotate(np.pi,[0,0]).translate(x_pos+(i+1)*x_increment,y_pos)
				bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, bottom_electrode_JJ_line_pos, "not", layer=self.bottom_layer)

				cell_out.add(top_electrode_JJ_line.copy().rotate(np.pi,[0,0]).translate(x_pos-JJ_line_inst.top_spacing_jj/2+(i+1)*x_increment,y_pos))
		...
		center_pad_1_pos = gdstk.rectangle((-litho_overlap/2,-2), (PPC_ground_inst.height_capa+2*PPC_ground_inst.electrode_height_difference+litho_overlap/2,2), layer=self.bottom_layer)
		center_pad_2_pos = gdstk.rectangle((-2,-(PPC_ground_inst.height_capa/2+PPC_ground_inst.electrode_height_difference+add_spacing_capa)-litho_overlap/2), (2,PPC_ground_inst.height_capa/2+PPC_ground_inst.electrode_height_difference+add_spacing_jj+litho_overlap/2), layer=self.bottom_layer)

		bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, center_pad_1_pos.copy().translate(x_pos,y_pos), "not", layer=self.bottom_layer)
		bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, center_pad_2_pos.copy().translate(x_pos+PPC_ground_inst.height_capa/2+PPC_ground_inst.electrode_height_difference,y_pos), "not", layer=self.bottom_layer)

		# add PPC to ground
		x_pos += PPC_ground_inst.electrode_height_difference+PPC_ground_inst.height_capa/2
		y_pos -= PPC_line_inst.height_capa/2+PPC_line_inst.electrode_height_difference+add_spacing_capa

		bottom_electrode_capa_ground = gdstk.rectangle((-litho_overlap/2,-(PPC_ground_inst.height_capa/2+PPC_ground_inst.electrode_height_difference)), (PPC_ground_inst.top_spacing_capa/2+PPC_ground_inst.width_capa,(PPC_ground_inst.height_capa/2+PPC_ground_inst.electrode_height_difference)), layer=self.bottom_layer)
		top_electrode_capa_ground = gdstk.rectangle((0,-PPC_ground_inst.height_capa/2), (PPC_ground_inst.bottom_spacing_capa/2+PPC_ground_inst.width_capa+litho_overlap/2,PPC_ground_inst.height_capa/2), layer=self.capa_top_layer)

		y_increment = PPC_ground_inst.x_size_of_cell
		for i in range(number_of_capacitors_ground):
			if i%2 == 0:
				bottom_electrode_capa_ground_pos = bottom_electrode_capa_ground.copy().rotate(-np.pi/2,[0,0]).translate(x_pos,y_pos-i*y_increment)
				bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, bottom_electrode_capa_ground_pos, "not", layer=self.bottom_layer)

				cell_out.add(top_electrode_capa_ground.copy().rotate(-np.pi/2,[0,0]).translate(x_pos,y_pos-PPC_ground_inst.top_spacing_capa/2-i*y_increment))

			else:
				bottom_electrode_capa_ground_pos = bottom_electrode_capa_ground.copy().rotate(np.pi/2,[0,0]).translate(x_pos,y_pos-(i+1)*y_increment)
				bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, bottom_electrode_capa_ground_pos, "not", layer=self.bottom_layer)

				cell_out.add(top_electrode_capa_ground.copy().rotate(np.pi/2,[0,0]).translate(x_pos,y_pos+PPC_ground_inst.top_spacing_capa/2-(i+1)*y_increment))

		# add chain of junctions to ground
		x_pos += 0
		y_pos += add_spacing_capa + PPC_line_inst.height_capa+2*PPC_line_inst.electrode_height_difference

		bottom_electrode_JJ_ground = gdstk.rectangle((-litho_overlap/2,-(JJ_ground_inst.height_jj/2+JJ_ground_inst.electrode_height_difference)), (JJ_ground_inst.top_spacing_jj/2+JJ_ground_inst.width_jj,(JJ_ground_inst.height_jj/2+JJ_ground_inst.electrode_height_difference)), layer=self.bottom_layer)
		top_electrode_JJ_ground = gdstk.rectangle((0,-JJ_ground_inst.height_jj/2), (JJ_ground_inst.bottom_spacing_jj/2+JJ_ground_inst.width_jj+litho_overlap/2,JJ_ground_inst.height_jj/2), layer=self.jj_top_layer)

		y_increment = JJ_ground_inst.x_size_of_cell
		for i in range(number_of_junctions_ground):
			if i%2 == 0:
				bottom_electrode_JJ_ground_pos = bottom_electrode_JJ_ground.copy().rotate(np.pi/2,[0,0]).translate(x_pos,y_pos+i*y_increment)
				bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, bottom_electrode_JJ_ground_pos, "not", layer=self.bottom_layer)

				cell_out.add(top_electrode_JJ_ground.copy().rotate(np.pi/2,[0,0]).translate(x_pos,y_pos+JJ_ground_inst.top_spacing_jj/2+i*y_increment))

			elif i == number_of_junctions_ground-1:
				bottom_electrode_JJ_ground_pos = bottom_electrode_JJ_ground.copy().scale(1, -1, (0, 0)).rotate(-np.pi/2,[0,0]).translate(x_pos,y_pos+(i+1)*y_increment)
				bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, bottom_electrode_JJ_ground_pos, "not", layer=self.bottom_layer)

				cell_out.add(top_electrode_JJ_ground.copy().rotate(-np.pi/2,[0,0]).translate(x_pos,y_pos-JJ_ground_inst.top_spacing_jj/2+(i+1)*y_increment))
		...
		for poly in bottom_electrode_neg:
			cell_out.add(poly.copy())
...
class CRLH_V04_cell:
	...
	def generateCell(self, **kwargs):
		...
		bottom_electrode_neg = gdstk.rectangle((-litho_overlap/2,-self.y_size_of_cell/2),(self.x_size_of_cell+litho_overlap/2,self.y_size_of_cell/2), layer=self.bottom_layer)
		top_electrode_capa_neg = gdstk.rectangle((-litho_overlap/2,-self.y_size_of_cell/2),(self.x_size_of_cell+litho_overlap/2,self.y_size_of_cell/2), layer=self.capa_top_layer)

		x_pos = number_of_capacitors_line*PPC_line_inst.x_size_of_cell + number_of_junctions_line*JJ_line_inst.x_size_of_cell + max_width_capa_ground + self.top_spacing_capa_ground + length_feedline/2
		y_pos = width_feedline/2

		top_electrode_capa_remove = gdstk.rectangle((0,0),(JJ_ground_inst.height_jj+2*2.0, JJ_ground_inst.width_jj+2.0), layer=self.capa_top_layer)
		top_electrode_capa_remove_pos = top_electrode_capa_remove.copy().translate(x_pos-(JJ_ground_inst.height_jj/2+2.0),y_pos+size_jj_to_ground)
		top_electrode_capa_neg = gdstk.boolean(top_electrode_capa_neg, top_electrode_capa_remove_pos, 'or', layer=self.capa_top_layer)
		...
		for i in range(number_of_capacitors_line):
			if i%2 == 0:
				...
				y_pos += 0
				top_electrode_capa_line_pos = top_electrode_capa_line.copy().rotate(0,[0,0]).translate(x_pos+i*x_increment,y_pos)
				top_electrode_neg = gdstk.boolean(top_electrode_capa_neg, top_electrode_capa_line_pos, "not", layer=self.capa_top_layer)
		...
		for i in range(number_of_junctions_line):
			if i%2 == 0:
				...
				x_pos += JJ_line_inst.top_spacing_jj/2
				y_pos = 0
				cell_out.add(top_electrode_JJ_line.copy().rotate(0,[0,0]).translate(x_pos+i*x_increment,y_pos))
				...
				x_pos -= JJ_line_inst.top_spacing_jj/2
				y_pos = 0
				cell_out.add(top_electrode_JJ_line.copy().rotate(np.pi,[0,0]).translate(x_pos+(i+1)*x_increment,y_pos))
		...
		bottom_electrode_capa_ground = gdstk.rectangle((-litho_overlap/2,-PPC_ground_inst.height_capa/2), (max_width_capa_ground+self.top_spacing_capa_ground+litho_overlap/2,PPC_ground_inst.height_capa/2), layer=self.bottom_layer)
		bottom_electrode_capa_ground_pos = bottom_electrode_capa_ground.translate(x_pos,y_pos)
		...
		feedline = gdstk.rectangle((-litho_overlap/2,-(width_feedline/2)), (length_feedline+litho_overlap/2,width_feedline/2), layer=self.bottom_layer)
		feedline_pos = feedline.translate(x_pos,y_pos)

		bottom_electrode_neg = gdstk.boolean(bottom_electrode_neg, feedline_pos, "not", layer=self.bottom_layer)
		...
		for i in range(number_of_junctions_ground):
			if i%2 == 0:
				...
				x_pos += 0
				y_pos += JJ_ground_inst.top_spacing_jj/2
				cell_out.add(top_electrode_JJ_ground.copy().rotate(np.pi/2,[0,0]).translate(x_pos,y_pos+i*y_increment))
				...
				x_pos += 0
				y_pos -= JJ_ground_inst.top_spacing_jj/2
				cell_out.add(top_electrode_JJ_ground.copy().rotate(-np.pi/2,[0,0]).translate(x_pos,y_pos+(i+1)*y_increment))
				# top_electrode_JJ_ground_pos = top_electrode_JJ_ground.copy().scale(1, -1, (0, 0)).rotate(-np.pi/2,[0,0]).translate(x_pos,y_pos+(i+1)*y_increment)
...
class Resistor_cell:
	...
	Class to generate the unit cell for a Composite Right-Left-Handed Josephson Transmission Line
	  There is one function:
		2) generateCell: generate the unit cell of a resistor.
		The variable self.x_size_of_cell and self.y_size_of_cell store sizes of the structure for global use
		Note that it stores the variables self.x_size_of_cell and self.y_size_of_cell of the classes Overlap_jj and Overlap_capa in the
		variables:
		1) x_size_of_resistor
		2) y_size_of_resistor
	'''
	...
	def __init__(self):
		...
		self.ground_layer = 1
		self.feedline_layer = 2
		self.resistor_layer = 3

		self.cells_in_element = 1

	def generateCell(self, **kwargs):

		resistor_inst = Resistor()
		resistor_inst.litho_overlap = self.litho_overlap

		resistor_inst.length = self.length_resistor
		resistor_inst.width = self.width_resistor
		
		self.resistor = resistor_inst.generateCell()

		litho_overlap = self.litho_overlap

		# generate cell to add features to
		cell_out = gdstk.Cell("Resistor_cell")

		# add resistor
		x_pos = 0
		y_pos = 0

		resistor_ref = gdstk.Reference(self.resistor, origin=(x_pos,y_pos), rotation=0)
		cell_out.add(resistor_ref)

		x_pos += resistor_inst.length
		y_pos += resistor_inst.width/2
		...
		# add ground
		x_ground = x_pos
		y_ground = resistor_inst.y_size_of_cell/2 + 2

		ground = gdstk.rectangle((-litho_overlap/2,-y_ground), (x_ground+litho_overlap/2,y_ground), layer=self.ground_layer)
...
if __name__ == '__main__':
	...
	test_SNAILTWPA = True
	test_LHTWPA = True
	test_CRLH_V01_TWPA = True
	test_CRLH_V02_TWPA = True
	test_CRLH_V03_TWPA = True
	test_CRLH_V04_TWPA = True
	test_resistor = True
	...
	if test_CRLH_V01_TWPA:
		lib = gdstk.Library()

		cell_out = gdstk.Cell("CRLH_V01_cell")

		CRLH_V01_JTL_inst = CRLH_V01_cell()

		CRLH_V01_JTL_inst.litho_overlap = 0.1
		CRLH_V01_JTL_inst.etching_offset = 0.0
		CRLH_V01_JTL_inst.y_low_current_ground = 3.0

		CRLH_V01_JTL_inst.width_jj_line = 1.0
		CRLH_V01_JTL_inst.height_jj_line = 4.3
		CRLH_V01_JTL_inst.spacing_jj_line = 1.0
		CRLH_V01_JTL_inst.bottom_spacing_jj_line = 1.0
		CRLH_V01_JTL_inst.top_spacing_jj_line = 1.0
		CRLH_V01_JTL_inst.electrode_height_difference_JJ_line = 0.5
		CRLH_V01_JTL_inst.number_of_junctions_line = 10
		
		CRLH_V01_JTL_inst.width_capa_line = 5.38
		CRLH_V01_JTL_inst.height_capa_line = 20.0
		CRLH_V01_JTL_inst.spacing_capa_line = 1.0
		CRLH_V01_JTL_inst.bottom_spacing_capa_line = 1.0
		CRLH_V01_JTL_inst.top_spacing_capa_line = 1.0
		CRLH_V01_JTL_inst.electrode_height_difference_capa_line = 0.5
		CRLH_V01_JTL_inst.number_of_capacitors_line = 2

		CRLH_V01_JTL_inst.width_jj_ground = 1.0
		CRLH_V01_JTL_inst.height_jj_ground = 4.3
		CRLH_V01_JTL_inst.spacing_jj_ground = 1.0
		CRLH_V01_JTL_inst.bottom_spacing_jj_ground = 1.0
		CRLH_V01_JTL_inst.top_spacing_jj_ground = 1.0
		CRLH_V01_JTL_inst.electrode_height_difference_JJ_ground = 0.5
		CRLH_V01_JTL_inst.number_of_junctions_ground = 10
		
		CRLH_V01_JTL_inst.width_capa_ground = 5.38
		CRLH_V01_JTL_inst.height_capa_ground = 20.0
		CRLH_V01_JTL_inst.spacing_capa_ground = 1.0
		CRLH_V01_JTL_inst.bottom_spacing_capa_ground = 1.0
		CRLH_V01_JTL_inst.top_spacing_capa_ground = 1.0
		CRLH_V01_JTL_inst.electrode_height_difference_capa_ground = 0.5
		CRLH_V01_JTL_inst.number_of_capacitors_ground = 2

		CRLH_V01_JTL = CRLH_V01_JTL_inst.generateCell()

		for i in range(1):
			if i%2 == 0:
				CRLH_V01_JTL_ref = gdstk.Reference(CRLH_V01_JTL, origin=(i*CRLH_V01_JTL_inst.x_size_of_cell,0))
			else:
				CRLH_V01_JTL_ref = gdstk.Reference(CRLH_V01_JTL, origin=(i*CRLH_V01_JTL_inst.x_size_of_cell,0), x_reflection=True)
			cell_out.add(CRLH_V01_JTL_ref)

		cell_out.flatten()
		lib.add(cell_out)
		lib.write_gds('test_elements/CRLH_V01.gds')


	if test_CRLH_V02_TWPA:
		lib = gdstk.Library()

		cell_out = gdstk.Cell("CRLH_V02_cell")

		CRLH_V02_JTL_inst = CRLH_V02_cell()

		CRLH_V02_JTL_inst.litho_overlap = 0.1
		CRLH_V02_JTL_inst.etching_offset = 0.0
		CRLH_V02_JTL_inst.y_low_current_ground = 3.0

		CRLH_V02_JTL_inst.width_jj_line = 1.0
		CRLH_V02_JTL_inst.height_jj_line = 4.3
		CRLH_V02_JTL_inst.spacing_jj_line = 1.0
		CRLH_V02_JTL_inst.bottom_spacing_jj_line = 1.0
		CRLH_V02_JTL_inst.top_spacing_jj_line = 1.0
		CRLH_V02_JTL_inst.electrode_height_difference_JJ_line = 0.5
		CRLH_V02_JTL_inst.number_of_junctions_line = 10
		
		CRLH_V02_JTL_inst.width_capa_line = 6.7
		CRLH_V02_JTL_inst.height_capa_line = 20.0
		CRLH_V02_JTL_inst.spacing_capa_line = 1.0
		CRLH_V02_JTL_inst.bottom_spacing_capa_line = 1.0
		CRLH_V02_JTL_inst.top_spacing_capa_line = 1.0
		CRLH_V02_JTL_inst.electrode_height_difference_capa_line = 0.5
		CRLH_V02_JTL_inst.number_of_capacitors_line = 2

		CRLH_V02_JTL_inst.width_jj_ground = 1.0
		CRLH_V02_JTL_inst.height_jj_ground = 4.3
		CRLH_V02_JTL_inst.spacing_jj_ground = 1.0
		CRLH_V02_JTL_inst.bottom_spacing_jj_ground = 1.0
		CRLH_V02_JTL_inst.top_spacing_jj_ground = 1.0
		CRLH_V02_JTL_inst.electrode_height_difference_JJ_ground = 0.5
		CRLH_V02_JTL_inst.number_of_junctions_ground = 10
		
		CRLH_V02_JTL_inst.width_capa_ground = 3.35
		CRLH_V02_JTL_inst.height_capa_ground = 20.0
		CRLH_V02_JTL_inst.spacing_capa_ground = 1.0
		CRLH_V02_JTL_inst.bottom_spacing_capa_ground = 1.0
		CRLH_V02_JTL_inst.top_spacing_capa_ground = 1.0
		CRLH_V02_JTL_inst.electrode_height_difference_capa_ground = 0.5
		CRLH_V02_JTL_inst.number_of_capacitors_ground = 2

		CRLH_V02_JTL = CRLH_V02_JTL_inst.generateCell()

		for i in range(1):
			if i%2 == 0:
				CRLH_V02_JTL_ref = gdstk.Reference(CRLH_V02_JTL, origin=(i*CRLH_V02_JTL_inst.x_size_of_cell,0))
			else:
				CRLH_V02_JTL_ref = gdstk.Reference(CRLH_V02_JTL, origin=(i*CRLH_V02_JTL_inst.x_size_of_cell,0), x_reflection=True)
			cell_out.add(CRLH_V02_JTL_ref)

		cell_out.flatten()
		lib.add(cell_out)
		lib.write_gds('test_elements/CRLH_V02.gds')

	
	if test_CRLH_V03_TWPA:
		lib = gdstk.Library()

		cell_out = gdstk.Cell("CRLH_V03_cell")

		CRLH_V03_JTL_inst = CRLH_V03_cell()

		CRLH_V03_JTL_inst.litho_overlap = 0.1
		CRLH_V03_JTL_inst.etching_offset = 0.0
		CRLH_V03_JTL_inst.y_low_current_ground = 3.0

		CRLH_V03_JTL_inst.width_jj_line = 1.0
		CRLH_V03_JTL_inst.height_jj_line = 4.3
		CRLH_V03_JTL_inst.spacing_jj_line = 1.0
		CRLH_V03_JTL_inst.bottom_spacing_jj_line = 1.0
		CRLH_V03_JTL_inst.top_spacing_jj_line = 1.0
		CRLH_V03_JTL_inst.electrode_height_difference_JJ_line = 0.5
		CRLH_V03_JTL_inst.number_of_junctions_line = 10
		
		CRLH_V03_JTL_inst.width_capa_line = 6.7
		CRLH_V03_JTL_inst.height_capa_line = 20.0
		CRLH_V03_JTL_inst.spacing_capa_line = 1.0
		CRLH_V03_JTL_inst.bottom_spacing_capa_line = 1.0
		CRLH_V03_JTL_inst.top_spacing_capa_line = 1.0
		CRLH_V03_JTL_inst.electrode_height_difference_capa_line = 0.5
		CRLH_V03_JTL_inst.number_of_capacitors_line = 2

		CRLH_V03_JTL_inst.width_jj_ground = 1.0
		CRLH_V03_JTL_inst.height_jj_ground = 4.3
		CRLH_V03_JTL_inst.spacing_jj_ground = 1.0
		CRLH_V03_JTL_inst.bottom_spacing_jj_ground = 1.0
		CRLH_V03_JTL_inst.top_spacing_jj_ground = 1.0
		CRLH_V03_JTL_inst.electrode_height_difference_JJ_ground = 0.5
		CRLH_V03_JTL_inst.number_of_junctions_ground = 10
		
		CRLH_V03_JTL_inst.width_capa_ground = 3.35
		CRLH_V03_JTL_inst.height_capa_ground = 20.0
		CRLH_V03_JTL_inst.spacing_capa_ground = 1.0
		CRLH_V03_JTL_inst.bottom_spacing_capa_ground = 1.0
		CRLH_V03_JTL_inst.top_spacing_capa_ground = 1.0
		CRLH_V03_JTL_inst.electrode_height_difference_capa_ground = 0.5
		CRLH_V03_JTL_inst.number_of_capacitors_ground = 2

		CRLH_V03_JTL = CRLH_V03_JTL_inst.generateCell()

		for i in range(1):
			if i%2 == 0:
				CRLH_V03_JTL_ref = gdstk.Reference(CRLH_V03_JTL, origin=(i*CRLH_V03_JTL_inst.x_size_of_cell,0))
			else:
				CRLH_V03_JTL_ref = gdstk.Reference(CRLH_V03_JTL, origin=(i*CRLH_V03_JTL_inst.x_size_of_cell,0), x_reflection=True)
			cell_out.add(CRLH_V03_JTL_ref)

		cell_out.flatten()
		lib.add(cell_out)
		lib.write_gds('test_elements/CRLH_V03.gds')


	if test_CRLH_V04_TWPA:
		lib = gdstk.Library()

		cell_out = gdstk.Cell("CRLH_V04_cell")

		CRLH_V04_JTL_inst = CRLH_V04_cell()

		CRLH_V04_JTL_inst.litho_overlap = 0.1
		CRLH_V04_JTL_inst.etching_offset = 0.0
		CRLH_V04_JTL_inst.y_low_current_ground = 3.0

		CRLH_V04_JTL_inst.width_jj_line = 1.0
		CRLH_V04_JTL_inst.height_jj_line = 4.3
		CRLH_V04_JTL_inst.spacing_jj_line = 1.0
		CRLH_V04_JTL_inst.bottom_spacing_jj_line = 1.0
		CRLH_V04_JTL_inst.top_spacing_jj_line = 1.0
		CRLH_V04_JTL_inst.electrode_height_difference_JJ_line = 0.5
		CRLH_V04_JTL_inst.number_of_junctions_line = 10
		
		CRLH_V04_JTL_inst.width_capa_line = 6.7
		CRLH_V04_JTL_inst.height_capa_line = 20.0
		CRLH_V04_JTL_inst.spacing_capa_line = 1.0
		CRLH_V04_JTL_inst.bottom_spacing_capa_line = 1.0
		CRLH_V04_JTL_inst.top_spacing_capa_line = 1.0
		CRLH_V04_JTL_inst.electrode_height_difference_capa_line = 0.5
		CRLH_V04_JTL_inst.number_of_capacitors_line = 2

		CRLH_V04_JTL_inst.width_jj_ground = 1.0
		CRLH_V04_JTL_inst.height_jj_ground = 4.3
		CRLH_V04_JTL_inst.spacing_jj_ground = 1.0
		CRLH_V04_JTL_inst.bottom_spacing_jj_ground = 1.0
		CRLH_V04_JTL_inst.top_spacing_jj_ground = 1.0
		CRLH_V04_JTL_inst.electrode_height_difference_JJ_ground = 0.5
		CRLH_V04_JTL_inst.number_of_junctions_ground = 10
		
		CRLH_V04_JTL_inst.width_capa_ground = 3.35
		CRLH_V04_JTL_inst.height_capa_ground = 20.0
		CRLH_V04_JTL_inst.spacing_capa_ground = 1.0
		CRLH_V04_JTL_inst.bottom_spacing_capa_ground = 1.0
		CRLH_V04_JTL_inst.top_spacing_capa_ground = 1.0
		CRLH_V04_JTL_inst.electrode_height_difference_capa_ground = 0.5
		CRLH_V04_JTL_inst.number_of_capacitors_ground = 2

		CRLH_V04_JTL = CRLH_V04_JTL_inst.generateCell()

		for i in range(1):
			if i%2 == 0:
				CRLH_V04_JTL_ref = gdstk.Reference(CRLH_V04_JTL, origin=(i*CRLH_V04_JTL_inst.x_size_of_cell,0))
			else:
				CRLH_V04_JTL_ref = gdstk.Reference(CRLH_V04_JTL, origin=(i*CRLH_V04_JTL_inst.x_size_of_cell,0), x_reflection=True)
			cell_out.add(CRLH_V04_JTL_ref)

		cell_out.flatten()
		lib.add(cell_out)
		lib.write_gds('test_elements/CRLH_V04.gds')
	...
	if test_resistor:
		...
		resistor_inst = Resistor_cell()

		resistor_inst.litho_overlap = 0.1
		resistor_inst.length_resistor = 22.3
		resistor_inst.width_resistor = 3.5

		resistor = resistor_inst.generateCell()

		resistor_ref = gdstk.Reference(resistor, origin=(0, 0))

		cell_out.add(resistor_ref)
		cell_out.flatten()
		lib.add(cell_out)
		lib.write_gds('test_elements/Resistor.gds')
