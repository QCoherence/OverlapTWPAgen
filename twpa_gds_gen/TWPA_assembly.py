...
5) CRLH: chain of capacitors in serie with junctions array with capacitor and junctions array to ground
6) LER: a feedline with lumped element resonators coupled to it
...
import numpy as np
import datetime
from TWPA_elements import Pad, Pad_TWPA_inv_microstrip, Pad_TWPA_CRLH_V02, Pad_TWPA_CRLH_V03, Pad_TWPA_CRLH_V04
...
class TWPA_assembly():
    ...
    def __init__(self):
        ...
        self.y_pad_inv  = 100 # Do not change (100)
        self.x_pad_inv = 280 # Do not change (280)
        self.x_pad_inv_gap    = 10 # Do not change (10)
        self.y_pad_inv_gap    = 15 # Do not change (15)
        
        self.x_connecting_line = None
        self.y_connecting_line  = 4 # Do not change (4)
        self.connecting_line_gap    = 13 # Do not change (13)
        self.pad_line_overlap       = 2 # You can change but not a relevant parameter
        
        self.capacitance_along_connecting_line_width       = 0 # Do not change (2)
        self.capacitance_along_connecting_line_pitch_first = 90 # Do not change (90)
        self.capacitance_along_connecting_line_pitch       = 100 # Do not change (100)
        ...
        self.jj_top_layer = None
        self.capa_top_layer = None
        ...
        self.add_wet_etching_test = False
        self.add_label = True
    ...
    def generate_chains(self,n_unit_cells,chain_type):
        ...
        for i in range(n_unit_cells):
            if chain_type == 'device':
                ...
                elif self.device == 'CRLH_V01':
                    if i%2 == 0:
                        ref = gdstk.Reference(unit_cell, origin=(i*unit_cell_size,0))
                    else:
                        ref = gdstk.Reference(unit_cell, origin=(i*unit_cell_size,0), x_reflection=True)
                elif self.device == 'CRLH_V02':
                    if i%2 == 0:
                        ref = gdstk.Reference(unit_cell, origin=(i*unit_cell_size,0))
                    else:
                        ref = gdstk.Reference(unit_cell, origin=(i*unit_cell_size,0), x_reflection=True)
                elif self.device == 'CRLH_V03':
                    if i%2 == 0:
                        ref = gdstk.Reference(unit_cell, origin=(i*unit_cell_size,0))
                    else:
                        ref = gdstk.Reference(unit_cell, origin=(i*unit_cell_size,0), x_reflection=True)
                elif self.device == 'CRLH_V04':
                    if i%2 == 0:
                        ref = gdstk.Reference(unit_cell, origin=(i*unit_cell_size,0))
                    else:
                        ref = gdstk.Reference(unit_cell, origin=(i*unit_cell_size,0), x_reflection=True)
    ...
    def generate_TC(self,n_unit_cells,chain_type):
        ...
        common_wire = gdstk.rectangle((0,-width_of_connecting_wire/2), (length_of_connecting_wire+(100-n_unit_cells)*unit_cell_size/2+Pad_litho_overlap,width_of_connecting_wire/2), layer=connection_wire_bottom_layer)
        cell.add(common_wire.copy().translate(x_pos_tic,0))
        x_pos_tic += length_of_connecting_wire+(100-n_unit_cells)*unit_cell_size/2+Pad_litho_overlap-litho_overlap/2
        ...
        ### Add common connecting wires
        common_wire = gdstk.rectangle((0,(0-width_of_connecting_wire)/2), (length_of_connecting_wire+(100-n_unit_cells)*unit_cell_size/2+Pad_litho_overlap,(0+width_of_connecting_wire)/2), layer=connection_wire_bottom_layer)
        cell.add(common_wire.copy().translate(x_pos_tic,0))
        x_pos_tic += length_of_connecting_wire+(100-n_unit_cells)*unit_cell_size/2
    ...
    def generate_TC_block(self, chain_type):
        ...
        for i in range(n_chains_per_row):
            ...
            x_pos += 100*unit_cell_size+2.5*probe_pad_size+2*length_of_connecting_wire
    ...
    def generate_TC_N(self,n_unit_cells,chain_type):
        '''
        This function generates one test chain for junctions or capacitors
        '''
        cell = gdstk.Cell("test_chain")
        ...
        ### Add probe pad
        x_pos_tic = 0
        probe_pad = gdstk.rectangle((0,-probe_pad_size/2), (probe_pad_size,probe_pad_size/2), layer=bottom_layer)
        cell.add(probe_pad.copy().translate(x_pos_tic,0))
        x_pos_tic = probe_pad_size-Pad_litho_overlap

        ### Add common connecting wires
        common_wire = gdstk.rectangle((0,(0-width_of_connecting_wire)/2), (length_of_connecting_wire+(100-n_unit_cells)*unit_cell_size/2+Pad_litho_overlap,(0+width_of_connecting_wire)/2), layer=bottom_layer)
        cell.add(common_wire.copy().translate(x_pos_tic,0))
        x_pos_tic += length_of_connecting_wire+(100-n_unit_cells)*unit_cell_size/2+Pad_litho_overlap-litho_overlap/2
    ...
    def generate_TC_block_N(self, chain_type):
        '''
        This function generates the jj test chains block:
        1) The label top refers to the test chains in the top row
        2) The label bottom refers to the test chains in the bottom row
        ...
        x_pos = self.write_field*500 - (probe_pad_size + length_of_connecting_wire + 3*unit_cell_size) # probe_pad_size/2 + 100*unit_cell_size+2.5*probe_pad_size+2*length_of_connecting_wire
        y_pos = 0
        ...
        for i in range(n_chains_per_row):
            ...
            n_top = 2*n_elements + offset
            n_bottom = 2*(n_elements + offset)
            TC_top = self.generate_TC_N(n_top,chain_type)
            TC_bottom = self.generate_TC_N(n_bottom,chain_type)
            ref_TC_top = gdstk.Reference(TC_top,origin=(x_pos,probe_pad_size*0.75),rotation=0)
            ref_TC_bottom = gdstk.Reference(TC_bottom,origin=(x_pos,-probe_pad_size*0.75),rotation=0)
            cell.add(ref_TC_top)
            cell.add(ref_TC_bottom)
        ...
        y_ground_dim = 3*probe_pad_size
        x_ground_dim = x_pos + probe_pad_size/2
        tc_neg = gdstk.rectangle((0,-y_ground_dim/2), (x_ground_dim,y_ground_dim/2), layer=bottom_layer).translate(-x_pos_ground,0)
        bottom_electrodes = cell.get_polygons(layer=bottom_layer,datatype=0)
        top_electrodes = cell.get_polygons(layer=jj_top_layer,datatype=0)
        for polygon in bottom_electrodes:
            tc_neg = gdstk.boolean(tc_neg, polygon.copy(), 'not', layer=bottom_layer)
        for polygon in tc_neg:
            cell_ret.add(polygon.translate(0,-y_ground_dim))
        for polygon in top_electrodes:
            cell_ret.add(polygon.copy().translate(0,-y_ground_dim))
    ...
    def generate_markers(self):
        ...
        if self.device in ('CRLH_V03', 'CRLH_V04'):
            ground_exclusion_sq = gdstk.Polygon([(0,0), (0,150), (150,150), (150,0)], layer=bottom_layer)
    ...
    def generate_TWPA_GDS(self):
        ...
        bottom_spacing_jj = self.bottom_spacing_jj
        top_spacing_jj = self.top_spacing_jj
        spacing_capa = self.spacing_capa
        bottom_spacing_capa = self.bottom_spacing_capa
        top_spacing_capa = self.bottom_spacing_capa

        if self.device in ('RH','LH','CRLH_V01','CRLH_V02','CRLH_V03','CRLH_V04'):
        ...
        elif self.device in ('LH','CRLH_V01','CRLH_V02','CRLH_V03','CRLH_V04'):
        ...
        if self.device == 'RH':
            ...
            y_taper_connection = self.y_connecting_line # y size of the taper connecting the end of the arm to the beginning of the TWPA
            x_taper_connection = x_size_capa - top_spacing_capa/2
            x_connection = x_taper_connection # x dimension of the small square to fill the rightmost unit cell of the TWPA
            y_connection = y_taper_connection # y dimension of the small square to fill the rightmost unit cell of the TWPA
        elif self.device == 'CRLH_V02':
            y_taper_connection = self.y_connecting_line # y size of the taper connecting the end of the arm to the beginning of the TWPA
            x_taper_connection = 0
            x_connection = x_taper_connection # x dimension of the small square to fill the rightmost unit cell of the TWPA
            y_connection = y_taper_connection # y dimension of the small square to fill the rightmost unit cell of the TWPA
        elif self.device == 'CRLH_V03':
            y_taper_connection = self.y_connecting_line # y size of the taper connecting the end of the arm to the beginning of the TWPA
        ...
        elif self.device in ('CRLH_V01','CRLH_V02','CRLH_V03','CRLH_V04'):
            y_ground = abs(2*unit_cell_height)
            x_ground = chain_length + x_taper_connection
        elif self.device in ('SJ','SNAIL'):
    ...
    def generate_feedline_GDS(self, **kwargs):
        ...
        cell_save = gdstk.Cell("cell_out")
        cell = gdstk.Cell("cell")
        cell_tmp = gdstk.Cell("feedline_tmp")
        ...
        feedlines_ref = gdstk.Reference(cell_tmp, origin=(0,0))
        cell.add(feedlines_ref)
        ...
        ### Add R4t measurements of Chromium films
        if self.add_R4t:
            R4t = self.generate_R4t(Exclude_ground=True)
            ref_R4t = gdstk.Reference(R4t,origin=(self.x_R4t,self.y_R4t),rotation=0)
            cell.add(ref_R4t)
    ...
    def generate_TWPA_GDS_inv_microstrip_pad(self):
        ...
        spacing_capa = self.spacing_capa
        bottom_spacing_capa = self.bottom_spacing_capa
        top_spacing_capa = self.top_spacing_capa
        ...
        pad_bottom_layer = self.pad_bottom_layer
        connection_wire_bottom_layer = self.connection_wire_bottom_layer
        if self.device in ('RH','CRLH_V01','CRLH_V02','SJ','SNAIL'):
            bottom_layer = self.jj_bottom_layer
        elif self.device in ('LH'):
            bottom_layer = self.capa_bottom_layer
        elif self.device in ('CRLH_V03','CRLH_V04'):
            bottom_layer = self.bottom_layer
        ...
        if self.device == 'RH':
            ...
            y_taper_connection = self.y_connecting_line # y size of the feedline connecting the end of the arm to the beginning of the TWPA
            x_taper_connection = 10 + litho_overlap/2
            x_connection = x_taper_connection # x dimension of the small square to fill the rightmost unit cell of the TWPA
            y_connection = y_taper_connection # y dimension of the small square to fill the rightmost unit cell of the TWPA
        elif self.device == 'CRLH_V02':
            y_taper_connection = self.y_connecting_line # y size of the feedline connecting the end of the arm to the beginning of the TWPA
        ...
        if self.device == 'CRLH_V02':
            ...
            self.capacitance_along_connecting_line_width       = 3 # Do not change (2)
            self.capacitance_along_connecting_line_pitch_first = 98.5 # Do not change (90)
            self.capacitance_along_connecting_line_pitch       = 100 # Do not change (100)
            Pad_inst = Pad_TWPA_CRLH_V02()
        elif self.device in ('CRLH_V03'):
            self.y_pad_inv  = 100 # Do not change (100)
            self.x_pad_inv = 280 # Do not change (280)
            self.x_pad_inv_gap    = 10 # Do not change (10)
            self.y_pad_inv_gap    = 15 # Do not change (15)
            ...
            self.capacitance_along_connecting_line_pitch_first = 98.5 # Do not change (90)
            self.capacitance_along_connecting_line_pitch       = 100 # Do not change (100)
            Pad_inst = Pad_TWPA_CRLH_V03()
        elif self.device in ('CRLH_V04'):
            self.y_pad_inv  = 100 # Do not change (100)
            self.x_pad_inv = 280 # Do not change (280)
            self.x_pad_inv_gap    = 10 # Do not change (10)
            self.y_pad_inv_gap    = 15 # Do not change (15)
        ...
        if self.device == 'CRLH_V02':
            Pad_inst.layer_capa_top_electrode = self.capa_top_layer
            Pad_inst.layer_jj_top_electrode = self.jj_top_layer
        if self.device in ('CRLH_V03','CRLH_V04'):
            Pad_inst.layer_jj_top_electrode = self.jj_top_layer
            Pad_inst.layer_capa_top_electrode = self.capa_top_layer
        ...
        if self.device in ('RH','LH','CRLH_V01','CRLH_V02','CRLH_V03','CRLH_V04'):
            ...
            # total_length = (n_unit_cells-1)*unit_cell_size + x_size_of_cell_tot
            total_length = chain_length + 2*x_connection
             
            Pad_inst.connecting_line_length = (grid_size_um[0] - total_length - 2*(Pad_inst.bonding_pad_length + Pad_inst.x_dice_gap))/2 # Size of the right and left arm to center the TWPA

        elif self.device in ('SJ','SNAIL'):

            Pad_inst.connecting_line_length = (grid_size_um[0] - chain_length - 2*(Pad_inst.bonding_pad_length + Pad_inst.x_dice_gap))/2 # Size of the right and left arm to center the TWPA
        ...
        if self.device in ('RH','LH','CRLH_V01'):

            connection_left = gdstk.rectangle((-litho_overlap/2,-y_connection/2), (x_connection+litho_overlap/2,y_connection/2), layer=bottom_layer) # Adds a connection between the right taper and the beggining of the TWPA
            cell.add(connection_left.copy().translate(start_position_chain,grid_size_um[1]/2))

            start_position_chain += x_connection
            ...
            connection_left_top_electrode_capa = gdstk.rectangle((-litho_overlap/2,-unit_cell_height), (x_connection,unit_cell_height), layer=self.capa_top_layer)
            cell.add(connection_left_top_electrode_capa.copy().translate(start_position_chain,grid_size_um[1]/2))
            ...
            connection_left = gdstk.rectangle((-litho_overlap/2,-y_connection/2), (x_connection+litho_overlap/2,y_connection/2), layer=bottom_layer) # Adds a connection between the right taper and the beggining of the TWPA
            ground_window_left = gdstk.rectangle((-litho_overlap/2,-unit_cell_height/2), (x_connection,unit_cell_height/2), layer=bottom_layer)
            connection_left_neg = gdstk.boolean(ground_window_left.copy().translate(start_position_chain,grid_size_um[1]/2), connection_left.copy().translate(start_position_chain,grid_size_um[1]/2), 'not', layer=bottom_layer)
            
            for i in range(0,len(connection_left_neg)):
                cell.add(connection_left_neg[i].copy())
            ...
            connection_left = gdstk.rectangle((-litho_overlap/2,-y_connection/2), (x_connection+litho_overlap/2,y_connection/2), layer=bottom_layer) # Adds a connection between the right taper and the beggining of the TWPA
            ground_window_left = gdstk.rectangle((-litho_overlap/2,-unit_cell_height), (x_connection,unit_cell_height), layer=bottom_layer)
            connection_left_neg = gdstk.boolean(ground_window_left.copy().translate(start_position_chain,grid_size_um[1]/2), connection_left.copy().translate(start_position_chain,grid_size_um[1]/2), 'not', layer=bottom_layer)
        ...
        if self.device in ('RH','LH','CRLH_V01'):
            ...
            connection_right = gdstk.rectangle((-litho_overlap/2,-y_connection/2), (x_connection+litho_overlap/2,y_connection/2), layer=bottom_layer) # Adds a connction between the end of the TWPA and the right taper
            ...
            connection_right = gdstk.rectangle((-litho_overlap/2,-y_connection/2), (x_connection+litho_overlap/2,y_connection/2), layer=bottom_layer) # Adds a connction between the right taper and the beggining of the TWPA
            cell.add(connection_right.copy().translate(end_position_chain,grid_size_um[1]/2))

            connection_right_top_electrode_capa = gdstk.rectangle((0,-unit_cell_height), (x_connection+litho_overlap/2,unit_cell_height), layer=self.capa_top_layer)
            cell.add(connection_right_top_electrode_capa.copy().translate(end_position_chain,grid_size_um[1]/2))
            ...
            connection_right = gdstk.rectangle((-litho_overlap/2,-y_connection/2), (x_connection+litho_overlap/2,y_connection/2), layer=bottom_layer) # Adds a connction between the right taper and the beggining of the TWPA
            ground_window_right = gdstk.rectangle((-litho_overlap/2,-unit_cell_height/2), (x_connection,unit_cell_height/2), layer=bottom_layer)
            connection_right_neg = gdstk.boolean(ground_window_right.copy().translate(end_position_chain,grid_size_um[1]/2), connection_right.copy().translate(end_position_chain,grid_size_um[1]/2), 'not', layer=bottom_layer)
            
            for i in range(0,len(connection_right_neg)):
                cell.add(connection_right_neg[i].copy())
            ...
            connection_right = gdstk.rectangle((-litho_overlap/2,-y_connection/2), (x_connection+litho_overlap/2,y_connection/2), layer=bottom_layer) # Adds a connction between the right taper and the beggining of the TWPA
            ground_window_right = gdstk.rectangle((-litho_overlap/2,-unit_cell_height), (x_connection,unit_cell_height), layer=bottom_layer)
            connection_right_neg = gdstk.boolean(ground_window_right.copy().translate(end_position_chain,grid_size_um[1]/2), connection_right.copy().translate(end_position_chain,grid_size_um[1]/2), 'not', layer=bottom_layer)
        ...
        if self.device == 'RH':
            ...
            y_ground = abs(2*unit_cell_height)
            x_ground = chain_length + 2*x_connection
        elif self.device in ('SJ','SNAIL'):
            y_ground = unit_cell_height
            x_ground = spacing_jj/2

        if self.device in ('RH', 'LH', 'CRLH_V01', 'CRLH_V02', 'SJ', 'SNAIL'):
            ground = gdstk.rectangle((0,-y_ground/2), (x_ground,y_ground/2), layer=ground_layer)
            ground.translate(start_position_chain-x_connection,grid_size_um[1]/2)
        ...
        if self.generate_test_chains_jj:
            x_TC = self.x_TC_jj
            y_TC = self.y_TC_jj
            if self.device in ('CRLH_V01','CRLH_V02'):
                TC_block = self.generate_TC_block('jj')
            if self.device in ('CRLH_V03','CRLH_V04'):
                TC_block = self.generate_TC_block_N('jj')
            ref_TC_block = gdstk.Reference(TC_block,origin=(x_TC,y_TC),rotation=0)
            cell.add(ref_TC_block)
        ...
        if self.add_label:
            ...
            info_label = gdstk.text(self.dev_label, self.text_size, (x_TC,y_TC), layer=self.ground_layer)
            if self.device in ('CRLH_V03','CRLH_V04'):
                info_label = gdstk.text(self.dev_label, self.text_size, (x_TC,y_TC), layer=self.bottom_layer)
            cell.add(*info_label)  
            self.y_cumulative  += self.text_size+2*self.label_shift
