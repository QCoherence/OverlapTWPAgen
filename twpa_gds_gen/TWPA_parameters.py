...
class feedline_parameters:

	def __init__(self, fab_parameters, device_parameters):

		self.module = 'feedline_parameters'
		self.version = '1.0.0'
...
class CRLH_V01_parameters:

	def __init__(self, fab_parameters, device_parameters):

		self.module = 'CRLH_V01_parameters'
		self.version = '1.0.0'
		...
		self.Wj_line = device_parameters['junction_parameters']['width_of_junction_in_line']*1e-6 #m
		self.Hj_line = device_parameters['junction_parameters']['height_of_junction_in_line']*1e-6 #m

		self.Wj_ground = device_parameters['junction_parameters']['width_of_junction_to_ground']*1e-6 #m
		self.Hj_ground = device_parameters['junction_parameters']['height_of_junction_to_ground']*1e-6 #m

		self.Wc_line = device_parameters['capacitor_parameters']['width_of_capacitor_in_line']*1e-6 #m
		self.Hc_line = device_parameters['capacitor_parameters']['height_of_capacitor_in_line']*1e-6 #m

		self.Wc_ground = device_parameters['capacitor_parameters']['width_of_capacitor_to_ground']*1e-6 #m
		self.Hc_ground = device_parameters['capacitor_parameters']['height_of_capacitor_to_ground']*1e-6 #m
		
		self.Njj_line = device_parameters['junction_parameters']['number_of_junctions_per_unit_cell_in_line']
		self.Njj_ground = device_parameters['junction_parameters']['number_of_junctions_per_unit_cell_to_ground']
		self.Nc_line = device_parameters['capacitor_parameters']['number_of_capacitors_per_unit_cell_in_line']
		self.Nc_ground = device_parameters['capacitor_parameters']['number_of_capacitors_per_unit_cell_to_ground']
		self.Ncell = device_parameters['TWPA_parameters']['number_of_cells']
	...
	def get_params(self, **kwargs):
		...
		### Junction in line area
		Aj_line = self.Wj_line*self.Hj_line
		### Junction in line resistance at room temperature
		Rj_line = np.pi*self.Delta0/(2*Aj_line*self.j_c*self.buisson_factor)
		### Critical current in line
		Ic_line = Aj_line*self.j_c/self.heating_factor
		### Junction in line normal state resistance
		Rn_line = np.pi*self.Delta0/(2*Ic_line)
		### Josephson inductance in line
		Lj_line = (self.phi0/Ic_line)
		### Josephson Capacitance in line
		Cj_line = Aj_line*self.c_j
		### Josephson inductance in line of one unit cell
		Lj_cell_line = self.Njj_line*Lj_line
		### Josephson capacitance in line of one unit cell
		Cj_cell_line = Cj_line/self.Njj_line

		### Capacitor in line dimensions
		Ac_line = self.Wc_line*self.Hc_line
		### Line capacitance
		C_line = Ac_line*self.c_c/self.Nc_line

		### Junction to ground area
		Aj_ground = self.Wj_ground*self.Hj_ground
		### Junction to ground resistance at room temperature
		Rj_ground = np.pi*self.Delta0/(2*Aj_ground*self.j_c*self.buisson_factor)
		### Critical current to ground
		Ic_ground = Aj_ground*self.j_c/self.heating_factor
		### Junction to ground normal state resistance
		Rn_ground = np.pi*self.Delta0/(2*Ic_ground)
		### Josephson inductance to ground
		Lj_ground = (self.phi0/Ic_ground)
		### Josephson Capacitance to ground
		Cj_ground = Aj_ground*self.c_j
		### Josephson inductance to ground of one unit cell
		Lj_cell_ground = self.Njj_ground*Lj_ground
		### Josephson capacitance to ground of one unit cell
		Cj_cell_ground = Cj_ground/self.Njj_ground

		### Capacitor to ground dimensions
		Ac_ground = self.Wc_ground*self.Hc_ground
		### Ground capacitance
		C_ground = Ac_ground*self.c_c/self.Nc_ground
		...
		if print_params:
			...
			print('Number of junctions in line per cell: Njj_line = %.f' %(self.Njj_line))
			print('Number of junctions to ground per cell: Njj_ground = %.f' %(self.Njj_ground))
			print('Number of capacitors in line per cell: Ncapa_line = %.f' %(self.Nc_line))
			print('Number of capacitors to ground per cell: Ncapa_ground = %.f' %(self.Nc_ground))

			print('Junction in line dimensions: H = %.2f um | W = %.2f um | A = %.2f um^2' %(self.Hj_line*1e6, self.Wj_line*1e6, Aj_line*1e12))
			print('Room temp resistance in line: Rj = %.2f Ohm' %(Rj_line))
			print('Critical current in line: Ic = %.2f uA' %(Ic_line*1e6))
			print('Normal state resistance in line: Rn = %.2f Ohm' %(Rn_line))
			print('Josephson inductance in line: Lj_line = %.2f pH' %(Lj_line*1e12))
			print('Josephson capacitance in line: Cj_line = %.2f fF' %(Cj_line*1e15))
			print('Josephson inductance in line per unit cell: Lj_cell_line = %.2f pH' %(Lj_cell_line*1e12))
			print('Josephson capacitance in line per unit cell: Cj_cell_line = %.2f fF' %(Cj_cell_line*1e15))

			print('Junction to ground dimensions: H = %.2f um | W = %.2f um | A = %.2f um^2' %(self.Hj_ground*1e6, self.Wj_ground*1e6, Aj_ground*1e12))
			print('Room temp resistance to ground: Rj = %.2f Ohm' %(Rj_ground))
			print('Critical current to ground: Ic = %.2f uA' %(Ic_ground*1e6))
			print('Normal state resistance to ground: Rn = %.2f Ohm' %(Rn_ground))
			print('Josephson inductance to ground: Lj_ground = %.2f pH' %(Lj_ground*1e12))
			print('Josephson capacitance to ground: Cj_ground = %.2f fF' %(Cj_ground*1e15))
			print('Josephson inductance to ground per unit cell: Lj_cell_ground = %.2f pH' %(Lj_cell_ground*1e12))
			print('Josephson capacitance to ground per unit cell: Cj_cell_ground = %.2f fF' %(Cj_cell_ground*1e15))

			print('Capacitance in line: C_line = %.2f fF' %(C_line*1e15))
			print('Capacitor area in line: A_line = %.2f um^2' %(Ac_line*1e12))

			print('Capacitance to ground: C_ground = %.2f fF' %(C_ground*1e15))
			print('Capacitor area to ground: A_ground = %.2f um^2' %(Ac_ground*1e12))

			return self.Hj_line*1e6, self.Wj_line*1e6, self.Hj_ground*1e6, self.Wj_ground*1e6, self.Hc_line*1e6, self.Wc_line*1e6, self.Hc_ground*1e6, self.Wc_ground*1e6


class CRLH_V02_parameters:

	def __init__(self, fab_parameters, device_parameters):

		self.module = 'CRLH_V02_parameters'
		self.version = '1.0.0'
...
class CRLH_V03_parameters:

	def __init__(self, fab_parameters, device_parameters):

		self.module = 'CRLH_V03_parameters'
		self.version = '1.0.0'
...
class CRLH_V04_parameters:

	def __init__(self, fab_parameters, device_parameters):

		self.module = 'CRLH_V04_parameters'
		self.version = '1.0.0'
